...
parseSpreadsheet : String -> List (List Int)
parseSpreadsheet =
    String.split "\n" >> List.map (String.split " " >> List.filterMap (String.toInt >> Result.toMaybe))
...
checksum =
    List.map findRowDiff >> List.sum


findRowDiff : List Int -> Int
findRowDiff list =
    Maybe.map2 (-) (List.maximum list) (List.minimum list) |> Maybe.withDefault 0


makeAllPairs : List Int -> List ( Int, Int )
makeAllPairs =
    List.sort >> List.reverse >> makeHeadPairs


makeHeadPairs : List Int -> List ( Int, Int )
makeHeadPairs list =
    case list of
        [] ->
            []
        ...
        a :: tail ->
            (List.map ((,) a) tail) ++ (makeHeadPairs tail)


part2 : List (List Int) -> Int
part2 =
    List.map part2CalcLine >> List.sum


part2CalcLine : List Int -> Int
part2CalcLine =
    makeAllPairs >> List.filter isEvenlyDivisible >> List.map (uncurry (//)) >> List.sum
