...
import Parser.LanguageKit as PLKit
...
registerParser : Parser String
registerParser =
    PLKit.variable (Char.isLower) (Char.isLower) (Set.fromList [ "inc", "dec" ])
...
instructionListParser =
    Parser.repeat Parser.zeroOrMore instructionParser
...
testInstructions =
    Parser.run instructionListParser testInput


runInstruction : Instruction -> Registers -> Registers
runInstruction instruction registers =
    if instruction.condition (getRegister instruction.conditionalRegister registers) instruction.threshold then
        Dict.insert instruction.register (getRegister instruction.register registers |> (+) instruction.offset) registers
    else
        registers


runInstruction2 : Instruction -> ( Int, Registers ) -> ( Int, Registers )
runInstruction2 instruction ( highest, registers ) =
    if instruction.condition (getRegister instruction.conditionalRegister registers) instruction.threshold then
        (getRegister instruction.register registers |> (+) instruction.offset)
            |> (\newVal -> ( Basics.max newVal highest, Dict.insert instruction.register newVal registers ))
    else
        ( highest, registers )


computeRegisters instructions =
    List.foldl runInstruction Dict.empty instructions


computeRegisters2 =
    List.foldl runInstruction2 ( 0, Dict.empty )


testOutput =
    testInstructions |> Result.withDefault [] |> computeRegisters |> Dict.values |> List.maximum


output1 =
    input
        |> Parser.run instructionListParser
