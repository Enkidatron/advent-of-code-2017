...
listOfLists =
    input |> String.lines |> List.map (String.split " ")


supporters : List String
supporters =
    listOfLists |> List.filterMap List.head


supported : List String
supported =
    listOfLists |> List.concatMap (List.drop 1)


root : Set String
root =
    Set.diff (Set.fromList supporters) (Set.fromList supported)
    ...
    listOfLists
        |> List.filterMap
            (\list ->
                case list of
                    name :: weight :: sublist ->
                        weight |> String.toInt |> Result.map (\w -> ( name, ( w, sublist ) )) |> Result.toMaybe
...
findWeight name =
    case Dict.get name dict of
        ...
        Just ( weight, subnames ) ->
            weight + List.sum (List.map findWeight subnames)

        Nothing ->
            0


unbalancedSubTowers : List ( String, Int, List ( String, Int ) )
unbalancedSubTowers =
    dict
        |> Dict.toList
        |> List.map (\( name, ( weight, subnames ) ) -> ( name, findWeight name, List.map (\name -> ( name, findWeight name )) subnames ))
        |> List.filter (\( name, totalweight, sublist ) -> List.map Tuple.second sublist |> allSame |> not)
        |> List.sortBy (\( name, totalweight, _ ) -> totalweight)


allSame list =
    (List.Extra.unique list |> List.length) <= 1


problemProgram =
    unbalancedSubTowers |> List.head



-- Then I did a little bit of manual weight comparison
