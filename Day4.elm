...
phraseBook : List (List String)
phraseBook =
    input |> String.split "\n" |> List.map (String.trim >> String.split " ")


validPhraseCount : Int
validPhraseCount =
    phraseBook |> List.filter List.Extra.allDifferent |> List.length


isValidPart2 : List String -> Bool
isValidPart2 =
    List.map (String.toList >> List.sort) >> List.Extra.allDifferent


part2Count : Int
part2Count =
    phraseBook |> List.filter isValidPart2 |> List.length
